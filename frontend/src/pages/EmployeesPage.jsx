...
function EmployeesPage({ employees, loading, refreshing, error, onRetry, onSelectEmployee, selectedEmployeeId, onCreateEmployee }) {
  ...
  return (
    <div className="page-section">
      ...
      <div className="section-heading rich-heading"><div><p className="panel-label">Directory</p><h2>Employee identity directory</h2><p className="section-meta">{filtered.length} of {employees.length} employees visible.</p></div><div className="heading-actions"><div className="count-chip"><UserRoundCheck size={16} /> {employees.length} total</div><button type="button" className="primary-button" onClick={onCreateEmployee}>Create Employee</button></div></div>
